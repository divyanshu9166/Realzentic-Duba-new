...
const updateSettingsSchema = z.object({
  ...
  vatTrn: z.string().trim().refine(value => value === '' || /^\d{15}$/.test(value), 'UAE VAT TRN must contain exactly 15 digits').optional(),
  vatRate: z.number().min(0).max(100).optional(),
  taxInvoiceFooter: z.string().max(2000).optional(),
...
export async function getStoreSettings() {
  ...
  return {
    ...
    data: {
      ...
      taxInvoiceFooter: settings.taxInvoiceFooter,
