...
export const invoiceSchema = z.object({
  ...
  type: z.enum(['SERVICE', 'COMMISSION', 'RENT', 'SECURITY_DEPOSIT', 'CREDIT_NOTE', 'OTHER']).default('SERVICE'),
  status: z.enum(['DRAFT', 'ISSUED']).default('DRAFT'),
  supplyDate: z.string().trim().optional().or(z.literal('')),
  dueDate: z.string().trim().optional().or(z.literal('')),
  taxTreatment: z.enum(['STANDARD_5', 'ZERO_RATED', 'EXEMPT', 'OUT_OF_SCOPE']).default('STANDARD_5'),
...
export const commissionSchema = z.object({
  ...
  brokerageAgreementRef: z.string().trim().max(120).optional().or(z.literal('')),
  dldRegistrationDate: z.string().trim().optional().or(z.literal('')),
  payoutEligibleAt: z.string().trim().optional().or(z.literal('')),
  eligibilityNote: z.string().trim().max(1000).optional().or(z.literal('')),
